...
import io.pdf.core.aware.FilterMetadataAware;
...
public class PluginServletManager {
			...
			if(null!= mappings && !mappings.isEmpty()){
				for(Element mapping: mappings){
					...
					if(!hash.containsKey(name)){
						throw new BaseException("No filter definition: "+ name);
					}
		...
		if(null!= servets&& !servets.isEmpty()){
			...
			for(Element servlet: servets){
				...
				if(null!= startup && !startup.isEmpty()){
					if(startup.size()> 1){
						throw new BaseException("Reduplicative servlet attribute load-on-startup : "+ name);
					}
			...
			if(null!= mappings && !mappings.isEmpty()){
				for(Element mapping: mappings){
					...
					String pattern= ((Element)mapping.elements("url-pattern").get(0)).getTextTrim();
					if(!hash.containsKey(name)){
						throw new BaseException("No servlet definition: "+ name);
					}
					hash.get(name).addUrlPattern(pattern);
				}
			}
	...
	private void newListenerInstance(){
		...
		try {
			...
			throw new BaseException("New Plugin ["+pluginContext.getName()+"] Listener instance failure",e);
		} 
	}
	...
	private void newFilterInstance(){
		...
		try {
			...
			throw new BaseException("New Plugin ["+pluginContext.getName()+"] Filter instance failure",e);
		} 
	}
	...
	private void newServletInstance(){
		...
		try {
			...
			throw new BaseException("New Plugin ["+pluginContext.getName()+"] Servlet instance failure",e);
		} 
	}
